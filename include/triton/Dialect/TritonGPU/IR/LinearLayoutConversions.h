...
// It accompanies the AMDGPUInThreadTranspose pass to transpose K-major tensor
// into K-minor prior to writing into LDS (shared memory). This conversion
// treats the sizePerThread as a 2D matrix and has different access pattern.
//
...
// [4, 8], threadsPerWarp = [2, 32], warpsPerCTA = [8, 1], order = [1, 0]}>. Here
// since sizePerThread is 2D, there could be two ways to traverse it: along the
// dim of 8 or the dim of 4. The regular toLinearLayout() would go through it
// from the leading order, i.e. dim of 8, but since we want to transpose it
...
// register layout is created by newly introduced transposeND and the rest still
// comes from identityND
