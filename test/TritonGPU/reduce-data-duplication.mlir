...
// CHECK-DAG: #[[$shared1:.*]] = #triton_gpu.shared<{vec = 4, perPhase = 2, maxPhase = 8, order = [1, 0], hasLeadingOffset = false, inThreadTranspose = false}>
// CHECK-DAG: #[[$shared2:.*]] = #triton_gpu.shared<{vec = 1, perPhase = 1, maxPhase = 1, order = [1, 0], hasLeadingOffset = false, inThreadTranspose = false}>
// CHECK-LABEL: no_threadRake_shared
// CHECK: [[shared_ptr:%.*]] = triton_gpu.local_alloc {{.*}} -> !tt.memdesc<32x128xf16, #[[$shared2]], #triton_gpu.shared_memory>
// CHECK: [[opB:%.*]] = triton_gpu.local_load [[shared_ptr]] : !tt.memdesc<32x128xf16, #[[$shared2]], #triton_gpu.shared_memory> -> {{.*}}
// CHECK: {{.*}} = tt.dot {{.*}}, [[opB]], {{.*}}
#blocked = #triton_gpu.blocked<{sizePerThread = [1, 8], threadsPerWarp = [16, 4], warpsPerCTA = [8, 1], order = [1, 0]}>
#blocked1 = #triton_gpu.blocked<{sizePerThread = [1, 8], threadsPerWarp = [4, 16], warpsPerCTA = [1, 8], order = [1, 0]}>
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 8 : i32, triton_gpu.target = "hip:gfx942", "triton_gpu.threads-per-warp" = 64 : i32} {
  tt.func public @no_threadRake_shared(%arg0: !tt.ptr<f16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}, %arg1: !tt.ptr<f16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}) {
...
// CHECK: {{.*}} = tt.dot {{.*}}, [[opB]], {{.*}}
#blocked = #triton_gpu.blocked<{sizePerThread = [1, 8], threadsPerWarp = [16, 4], warpsPerCTA = [8, 1], order = [1, 0]}>
#blocked1 = #triton_gpu.blocked<{sizePerThread = [4, 8], threadsPerWarp = [4, 16], warpsPerCTA = [1, 8], order = [1, 0]}>
#mma = #triton_gpu.amd_mfma<{versionMajor = 3, versionMinor = 0, warpsPerCTA = [4, 2], instrShape = [32, 32], isTransposed = true}>
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 8 : i32, triton_gpu.target = "hip:gfx942", "triton_gpu.threads-per-warp" = 64 : i32} {
