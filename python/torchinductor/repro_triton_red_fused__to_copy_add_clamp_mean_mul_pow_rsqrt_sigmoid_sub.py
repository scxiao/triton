...
def main():
    ...
    if args.platform in ("amd", "both"):
        ...
        grid = (xnumel,)
        r = benchmark_kernel(triton_red_fused__to_copy_add_clamp_mean_mul_pow_rsqrt_sigmoid_sub_23, grid, [in0, in1, in2, xnumel, r0_numel], constexpr_kwargs, "AMD", iters=args.iters, profiling_mode=enabled, profiling_with_stack=with_stack)
    ...
    if args.platform in ("nv", "both"):
        ...
        print(f"  xnumel={xnumel}, r0_numel={r0_numel}, {constexpr_kwargs}")
        grid = (xnumel,)
        r = benchmark_kernel(triton_red_fused__to_copy_add_clamp_mean_mul_pow_rsqrt_sigmoid_sub_30, grid, [in0, in1, in2, xnumel, r0_numel], constexpr_kwargs, "NV", iters=args.iters, profiling_mode=enabled, profiling_with_stack=with_stack)
        print(f"  Min: {r['min']:.1f}us  Median: {r['median']:.1f}us  P10: {r['p10']:.1f}us  P90: {r['p90']:.1f}us  Iters: {r['iters']}")
        print()
