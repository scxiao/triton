...
def main():
    ...
    if args.platform in ("amd", "both"):
        ...
        grid = (xnumel,)
        r = benchmark_kernel(triton_poi_fused_add_arange_index_put_new_zeros_171, grid, [in0, out0, xnumel], constexpr_kwargs, "AMD", iters=args.iters)
    ...
    if args.platform in ("nv", "both"):
        ...
        print(f"  xnumel={xnumel}, {constexpr_kwargs}")
        grid = (xnumel,)
        r = benchmark_kernel(triton_poi_fused_add_arange_index_put_new_zeros_182, grid, [in0, out0, xnumel], constexpr_kwargs, "NV", iters=args.iters)
        print(f"  Min: {r['min']:.1f}us  Median: {r['median']:.1f}us  P10: {r['p10']:.1f}us  P90: {r['p90']:.1f}us  Iters: {r['iters']}")
        print()
