...
def do_bench_rotating_tensor(fn, warmup=25, rep=100, grad_to_none=None, quantiles=None, fast_flush=True, return_mode="mean"):
    """
    ...
    event_repeat = max(1, int(1 / estimate_ms))
    ...
    for i in range(n_repeat):
        ...
        for j in range(event_repeat):
            fn(j)
        end_event[i].record()
    # Record clocks
    torch.cuda.synchronize()
    times = torch.tensor([s.elapsed_time(e) / event_repeat for s, e in zip(start_event, end_event)], dtype=torch.float)
